...
class LogProcessingService:
    ...
    def process_and_save(self, request, ai_result):
        ...
        # 🌟 LẤY THÔNG TIN ERROR_TYPE TỪ REPOSITORY TẬP TRUNG
        error_type = self.error_type_repo.get_or_create_error_type(diagnosis_code, diagnosis_name)

        cache_key = f"{request.service_name}_{diagnosis_code}"
        now = datetime.now()
        ...
        if cache_key in LOCAL_ALERT_CACHE:
            ...
            if now < cached_data["expire_at"]:
                ...
                if cached_data["count"] % 10 == 0:
                    ...
                    # 🌟 ĐÃ SỬA: Thay thế diagnosis_code/name bằng error_type_id
        ...
        # 🌟 ĐÃ SỬA: Thay thế các cột lỗi cũ của bảng Incident bằng error_type_id
        new_incident = Incident(
            ...
            last_seen=now,
            occurrence_count=1,
            title=f"Phát hiện lỗi [{diagnosis_name}] bất thường!",
            recent_trace_ids=request.trace_id
        )
        self.db.add(new_incident)
        self.db.flush()

        # 🌟 ĐÃ SỬA: Thay thế các cột lỗi cũ của bảng AIPrediction bằng error_type_id
        new_prediction = AIPrediction(
            trace_id=request.trace_id,
